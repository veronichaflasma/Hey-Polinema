            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('feed') ? 'is-invalid' : '' }}" name="feed_id" id="feed_id" required>
                    @foreach($feeds as $id => $entry)
                        <option value="{{ $id }}" {{ (old('feed_id') ? old('feed_id') : $comment->feed->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('feed'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sender') ? 'is-invalid' : '' }}" name="sender_id" id="sender_id" required>
                    @foreach($senders as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sender_id') ? old('sender_id') : $comment->sender->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Comment::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $comment->status) === (string) $key ? 'checked' : '' }}>
