...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ownerFundRaiser.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.owner-fund-raisers.update", [$ownerFundRaiser->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $ownerFundRaiser->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('caption') ? 'is-invalid' : '' }}" type="text" name="caption" id="caption" value="{{ old('caption', $ownerFundRaiser->caption) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fund') ? 'is-invalid' : '' }}" type="number" name="fund" id="fund" value="{{ old('fund', $ownerFundRaiser->fund) }}" step="0.01">
                @if($errors->has('fund'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fund') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\OwnerFundRaiser::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $ownerFundRaiser->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('days') ? 'is-invalid' : '' }}" type="number" name="days" id="days" value="{{ old('days', $ownerFundRaiser->days) }}" step="1">
                @if($errors->has('days'))
                    <div class="invalid-feedback">
                        {{ $errors->first('days') }}
                    </div>
